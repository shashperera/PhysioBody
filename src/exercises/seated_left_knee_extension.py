...
def do_seated_left_knee_extension(humans: List, current_step: int) -> Tuple[int, str]:
    """Perform seated knee flexion and extension with the left leg.
...
def perform_step(human: Dict, current_step: int) -> Tuple[int, str]:
    ...
    2. Extend the left leg such that the inner angle is more than 180.
    ...
    if current_step == 0:
        ...
        if make_360(angle=angle) >= 180:
            return current_step + 1, 'Extension limit reached\nSlowly lower left leg'

        return current_step, 'Continue extending the left leg'
        ...
        if 0 < angle < 150:
            return current_step + 1, 'Left leg back in starting position'

        return current_step, 'Continue lowering the left leg'
...
def satisfies_prerequisites(humans: List) -> Tuple[bool, str]:
    ...
    3. Right leg in seated position.
    """
    if len(humans) == 0:
    ...
    if not (90 <= int(angle) <= 150):
        return False, 'Best done sitting in a chair. Bend the knee as far as possible and hold for 5sec then straighten as far as possible or bring back to start position. Slowly the range will improve. Right leg not seated'
